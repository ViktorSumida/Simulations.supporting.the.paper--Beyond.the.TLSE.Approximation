...
class Star:
    ...
    def __init__(self, raio, raioSun, intensidadeMaxima, coeficienteHum, coeficienteDois, coeficienteTres, coeficienteQuatro, tamanhoMatriz):
        ...
        self.estrelaMatriz = Star.criaEstrela(self.Nx, self.Ny, self.tamanhoMatriz, self.raio, self.intensidadeMaxima,
                                         self.coeficienteHum, self.coeficienteDois, self.coeficienteTres, self.coeficienteQuatro)
    ...
    @njit(parallel=True)
    def criaEstrela(lin, col, tamanhoMatriz, raio, intensidadeMaxima, 
                    coeficienteHum, coeficienteDois, coeficienteTres, coeficienteQuatro):
        """
        ...
        for i in prange(lin):
            for j in range(col):
                ...
                if dist <= raio:
                    ...
                    part1 = 1 - coeficienteHum * (1 - math.sqrt(cosTheta)) - coeficienteDois * (1 - cosTheta)
                    part2 = - coeficienteTres * (1 - cosTheta**1.5) - coeficienteQuatro * (1 - cosTheta**2.0)
    ...
    def criaRuidos(self, ruidos): 
        ...
        for ruido in ruidos:
            raio_mancha_pixel = self.raio * ruido.raio  # raio em função do raio da estrela em pixels
            ...
            yy = ys + self.Ny / 2  # posição em pixel relativa à origem da matriz
            xx = xs + self.Nx / 2
            ...
            ii, = np.where((((vx * np.cos(anguloRot) - vy * np.sin(anguloRot)) / np.cos(anguloHelio))**2 +
                            (vx * np.sin(anguloRot) + vy * np.cos(anguloRot))**2) < raio_mancha_pixel**2)
